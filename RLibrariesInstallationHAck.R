...
for (file in files) {
  ...
  l_temp_lib_list <-
    l_temp_lib_list[grepl('library', l_temp_lib_list, ignore.case = T)]
  
  #removing the extra library command from the R library loading command
  l_temp_lib_list <-
    sapply(l_temp_lib_list, function(p_library) {
      gsub("[\\(\\)]", "", regmatches(p_library, gregexpr("\\(.*?\\)", p_library))[[1]])
    })
...
tryCatch({
  ...
  error = function(cond) {
    print('There are some isntallation errors')
    print(cond)
  }
})
